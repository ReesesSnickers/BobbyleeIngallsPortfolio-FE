...
import BlogCardRecent from "../BlogCardRecent.js";

let props;
let mountedComponent;

describe("BlogCardRecent", () => {
  const testComponent = () => {
    if (!mountedComponent) {
      mountedComponent = mount(<BlogCardRecent {...props} />);
    }
  ...
  // initial render
  it("Renders the BlogCardRecent component", () => {
    let button = testComponent().find(
      "button.portfolio-blogcardrecent-wrapper"
    );
  ...
  it("should match snapshot", () => {
    const tree = renderer.create(<BlogCardRecent {...props} />).toJSON();
